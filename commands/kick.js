...
if(!user.kickable) return csybot.send({ embeds: [noperm] });
if(user.id == client.user.id || user.id == csybot.config.ownerid) return csybot.send({ embeds: [noperm] });
await user.kick();
...
const success = {
  ...
  description: csybot.lang(getlang, "kick_success", banneduser),
  footer: {
    text: csybot.footer
  },
